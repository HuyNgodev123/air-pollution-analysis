...
function Dashboard() {
  ...
  const renderContent = () => {
    if (loading) return <p className="text-center py-4">Đang tải dữ liệu từ vệ tinh...</p>;
    if (error) return <p className="text-center text-red-500">{error}</p>;
    
    // Nếu chưa chọn gì cả
    if (selectedCities.length === 0) {
      return <p className="text-center py-10 text-gray-500">Vui lòng tìm và chọn một địa điểm để xem dữ liệu.</p>;
    }

    // Nếu đã chọn nhưng API chưa trả về data (hoặc lỗi mạng)
    if (Object.keys(data).length === 0 && !loading) {
        return <p className="text-center text-gray-500">Không tìm thấy dữ liệu cho địa điểm này.</p>;
    }
    ...
    return (
      <div>
        ...
        {selectedCities.length === 1 && data[selectedCities[0].value] && (
          <PollutantDetails 
            data={data[selectedCities[0].value].current} 
          />
        )}
      ...
      <div id="dashboard">
        ...
        <div className="controls-grid">
          ...
          <div className="control-item location-item">
            <label className="invisible-label">GPS</label> {/* Label ẩn để căn dòng */}
            <button 
